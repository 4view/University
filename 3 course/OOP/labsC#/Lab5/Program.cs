...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        Console.WriteLine("=== Статические члены класса ===\n");

        // 6. Вывод количества объектов ДО создания объектов
        Console.WriteLine("1. ВЫВОД КОЛИЧЕСТВА ОБЪЕКТОВ ДО СОЗДАНИЯ:");
        Console.WriteLine("----------------------------------------");
        Console.WriteLine($"Количество созданных объектов Person: {Person.Count}");
        Console.WriteLine(
            $"Количество объектов в массиве Encyclopedia: {Encyclopedia.ObjectsInArrCount}"
        );

        // 7. Создание объектов классов-наследников
        Console.WriteLine("\n2. СОЗДАНИЕ ОБЪЕКТОВ КЛАССОВ-НАСЛЕДНИКОВ:");
        Console.WriteLine("----------------------------------------");

        var painter1 = new Painter(
            "FirstPainterName",
            "FirstPainterActType",
            "FirstPainterDescription",
            "FirstPainterStyle"
        );

        var painter2 = new Painter(
            "SecondPainterName",
            "SecondPainterActType",
            "SecondPainterDescription",
            "SecondPainterStyle"
        );
        ...
        var writer2 = new Writer(
            "SecondWriterName",
            "SecondWriterActType",
            "SecondWriterDescription",
            "SecondWriterStyle"
        );

        Console.WriteLine($"\nПосле создания 4 объектов:");
        Console.WriteLine($"Количество созданных объектов Person: {Person.Count}");

        // Создание энциклопедии и добавление объектов в массив
        Console.WriteLine("\n3. СОЗДАНИЕ ЭНЦИКЛОПЕДИИ И ДОБАВЛЕНИЕ ОБЪЕКТОВ:");
        Console.WriteLine("------------------------------------------------");

        var mainEnc = new Encyclopedia("История искусства", DateTime.Now.ToString("dd.MM.yy"));

        // Добавляем объекты в энциклопедию
        mainEnc.AddPerson(painter1);
        mainEnc.AddPerson(painter2);
        mainEnc.AddPerson(writer1);
        mainEnc.AddPerson(writer2);

        Console.WriteLine($"\nПосле добавления объектов в энциклопедию:");
        Console.WriteLine($"Количество созданных объектов Person: {Person.Count}");
        Console.WriteLine(
            $"Количество объектов в массиве Encyclopedia: {Encyclopedia.ObjectsInArrCount}"
        );
        Console.WriteLine($"Объектов в текущей энциклопедии: {mainEnc.PersonCount}");

        // Вывод всех данных энциклопедии
        Console.WriteLine("\n4. ВЫВОД ДАННЫХ ЭНЦИКЛОПЕДИИ:");
        Console.WriteLine("-----------------------------");
        mainEnc.PrintAll();

        // Демонстрация работы со статическими членами
        Console.WriteLine("\n5. ДЕМОНСТРАЦИЯ РАБОТЫ СО СТАТИЧЕСКИМИ ЧЛЕНАМИ:");
        Console.WriteLine("----------------------------------------------");

        // Доступ к статическим членам без создания объектов
        Console.WriteLine($"\nДоступ к статическим членам без объектов:");
        Console.WriteLine($"Person.GetCount(): {Person.GetCount()}");
        Console.WriteLine($"Person.Count: {Person.Count}");
        Console.WriteLine(
            $"Encyclopedia.GetObjectsInArrayCount(): {Encyclopedia.GetObjectsInArrCount()}"
        );
    }
}
